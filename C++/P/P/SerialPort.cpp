...
/** 当一个数据发送结束时，从data_buff中读出数据 */
char data_buff[256];
int temp_data_count = 0;
uchar origin_picture[4800];
bool recive_picture_flag=false;

void getDataToPicture(char* p);
...
void getDataToPicture(char *p)
{
	...
	if (cmd == "end")
	{
		if (recive_picture_flag)
		{
			showMyPicture();
		}
		recive_picture_flag = false;
	}
	else if (recive_picture_flag)
	{
		int data = atoi(p);
		...
		{
			int temp_count = my_picture_data_count - 4800;
		}
	}
	else if (cmd == "start")
	{
		my_picture_data_count = 0;
		recive_picture_flag = true;
	}
}


void getPictureFromCar(int portId)
{
	CSerialPort mySerialPort;
	if (!mySerialPort.InitPort(portId, 115200))
	{
